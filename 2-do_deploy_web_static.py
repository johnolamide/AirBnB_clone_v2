...
def do_deploy(archive_path):
    ...
    try:
        ...
        local("tar -xzf /tmp/{} -C {}/".format(archive_basename, release_path))

        run("rm /tmp/{}".format(archive_basename))

        local("rm /tmp/{}".format(archive_basename))
        ...
        local("rm -f {}".format(current_path))
        local("ln -s {}/ {}".format(release_path, current_path))
